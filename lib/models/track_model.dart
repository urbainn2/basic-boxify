...
class Track extends Equatable {
  ...
  @HiveField(24)
  final String? privateReleaseDate;
  ...
  Track copyWith({
    ...
    return Track(
      ...
      privateReleaseDate: privateReleaseDate ?? this.privateReleaseDate,
  ...
  factory Track.fromDocumentNoFuture(
    ...
    return Track(
        ...
        lyrics: data['lyrics'] ?? '',
        folder: data['folder'] ?? '',
        privateReleaseDate: getPrivateReleaseDate(data),
  ...
  /// Ensure privateReleaseDate is in the format 'yyyy-MM-dd'
  static String getPrivateReleaseDate(Map<String, dynamic> data) {
    // Directly return the current date if privateReleaseDate is not present or is null
    if (!data.containsKey('privateReleaseDate') ||
        data['privateReleaseDate'] == null) {
      return DateTime.now().toIso8601String().substring(0, 10);
    }

    var privateReleaseDate = data['privateReleaseDate'];

    // If privateReleaseDate is a Timestamp, convert to DateTime and format
    if (privateReleaseDate is Timestamp) {
      return privateReleaseDate.toDate().toIso8601String().substring(0, 10);
    }

    // If it's a string, ensure it's in the correct format, fallback to current date if not
    if (privateReleaseDate is String && privateReleaseDate.length == 10) {
  ...
  // /// ensure privateReleaseDate is in the format 'yyyy-MM-dd'
  // static String getPrivateReleaseDate(data) {
  //   var privateReleaseDate;

  //   // If it's in data
  //   if (data.containsKey('privateReleaseDate') &&
  //       data['privateReleaseDate'] != null) {
  //     privateReleaseDate = data['privateReleaseDate'];

  //     // if it's a valid string
  //     if (privateReleaseDate is String &&
  //         privateReleaseDate.isNotEmpty &&
  //         privateReleaseDate.length >= 10) {
  //       // if it's too long
  //       if (privateReleaseDate.length > 10) {
  //         // shorten it
  //         privateReleaseDate = privateReleaseDate.substring(0, 10);
  //       } else if (privateReleaseDate.length < 10) {
  //         // if it's too short
  //         privateReleaseDate =
  //             DateTime.now().toIso8601String().substring(0, 10);
  //       } else {
  //         privateReleaseDate = privateReleaseDate;
  //       }
  //     }
  //     // If it's a timestamp
  //     else if (privateReleaseDate is Timestamp) {
  //       privateReleaseDate =
  //           privateReleaseDate.toDate().toIso8601String().substring(0, 10);
  //     }
  //   }
  //   // if it's not in data
  //   else {
  //     final now = DateTime.now();
  //     privateReleaseDate = now.toIso8601String().substring(0, 10);
  //   }
  //   return privateReleaseDate;
  // }
